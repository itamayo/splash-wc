...
require('@wonkytech/tm-firebase-user');

document.addEventListener('user-logged-in', () => console.log('AAAAAAAAAAAAAAAA'));
window.customElements.define('tm-splash-screen', class extends litElement.LitElement {
  ...
  addLoginIfRequired() {
    ...
    if (login) {
      if (config) {
        return litHtml.html`<tm-firebase-user id="login" .config="${config}"></tm-firebase-user>`;
      } else {
        return litHtml.html`<tm-firebase-user id="login"></tm-firebase-user>`;
      }
    } else {
      return litHtml.html``;
    }
